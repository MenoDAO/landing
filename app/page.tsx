...
export default function Home() {
  ...
  // Suppress first paint until locale is confirmed — prevents English flash
  const [localeReady, setLocaleReady] = useState(false);

  useEffect(() => {
    // i18n module already called getInitialLocale() at load time,
    // but we still need to re-apply here in case the module was cached
    // with a stale language from a previous session on the same tab.
    const stored = localStorage.getItem("menodao_preferred_language");
  ...
  return (
    <I18nextProvider i18n={i18n}>
      {/* Hold render until locale is confirmed to prevent English flash */}
      {!localeReady ? null : (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      ...
                      <span className="text-sm font-bold text-green-600">
                        {t("testimonials.saved", {
                          amount: testimonial.savings,
                        })}
                      </span>
                    </div>
                  </div>
